	...
	cap noi {
		qui {
			...
			if ("`xlabel'"=="") {
				...
				if ("`zeroline'"!="" | "`zeroline_s'"!="") {
					if (`xlo'>0) {
						...
						if ((`oxlo'>(`xlo'+`xn2')) | (`oxlo'<(`xlo'-`xn2'))) {
							...
							/*
							else if (`oxlo'<(`xlo'-`xn2')) {
								if !mod(`xlo'*10^`prec',5) local xlo = round(`xlo'-`xn',5/10^`prec')
								else local xlo = round(`xlo'-`xn',5/10^`=`prec'+1')
							}
							*/
				...
				local xlabox "`oxlo' " " `oxhi' " ", tlwidth(none) custom add"
				if (!strpos(" `xlabel' "," 0 ") & ("`zeroline'"!="" | "`zeroline_s'"!="")) local xlabadd "0, tlength(*3.5) custom add"
				else local xlabadd "`: word 1 of `xlabel'' " ", tlength(*3.5) tlwidth(none) custom add"
			}
			...
			if ("`prnm'"!="") {
				...
				graph tw
					...
					||,
						...
						xlabel(`xlabel') xlabel(`xlabox') xlabel(`xlabadd') xline(`zeroline') xscale(`xscale') xsize(`xsize') xtitle(`xtitle')
