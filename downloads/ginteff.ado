	...
	cap noi {
		qui {
			...
			if (!_rc & "`sname'"!="0") {
				...
				_n " (To drop all, you can type {cmd:mata: mata drop __s_*} in Stata's command window.)"
				exit(198)
			}
			...
			else if ("`e(cmd)'"=="margins") {
				di as err "{bf:ginteff} cannot work with {bf:margins}' posted results"
				exit(198)
			}
			...
			if ("`prnm'"!="" & "`fvdx'"=="1") {
				...
				if (_rc==110) {
					di as err "a variable {it:`prnm'#} already exists;" _n " specify a different stub name in {bf:obseff()}"
					exit(198)
				}
				else if (_rc) {
